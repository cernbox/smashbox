...
class Client:
    ...
    def PROPFIND(
        ...
        if parse_check:
            if (
                200 <= r.status_code and r.status_code < 300
            ):  # only parse the reponse type for positive responses
                # TODO: multiple Content-Type response headers will confuse the client as well
                fatal_check(
                    "application/xml; charset=utf-8" in r.headers["Content-Type"],
                    'Wrong response header "Content-Type:%s"'
                    % r.headers["Content-Type"],
                )  # as of client 1.7 and 1.8
                r.propfind_response = _parse_propfind_response(
                    r.response_body, depth=depth
                )

        return r

    def PUT(
        self, fn: str, url: str, headers: Optional[dict] = None, offset: int = 0
    ) -> httpx.Response:
    ...
    def GET(
        self, url: str, fn: Optional[str] = None, headers: Optional[dict] = None
    ) -> httpx.Response:
    ...
    def MOVE(
        self, url: str, destination: str, overwrite: Optional[OverwriteType] = None
    ) -> httpx.Response:
    ...
    def _perform_request(
        ...
        res = self._client.request(
            method, url, content=content, headers=_headers, files=files
        )
        logger.debug(
            "_perform_request url=%s header=%s rc=%s reply_headers=%s",
            url,
            headers,
            res.status_code,
            res.headers,
...
def _parse_propfind_response(text: str, depth: Optional[PropfindDepth] = None) -> list:
    ...
    for child in root:
        ...
        allowed_children_tags(
            child, ["{DAV:}href", "{DAV:}propstat"]
        )  # only these elements are allowed

        fatal_check(
            len(child.findall("{DAV:}href")) == 1
        ...
        for propstat in child.findall("{DAV:}propstat"):
            ...
            for p in prop:
                ...
                fatal_check(
                    not rp.has_key(p.tag), p.tag
                )  # duplicate elements are not allowed
                if p.tag == "{DAV:}resourcetype":
                    fatal_check(
                        len(list(p)) in [0, 1]
                    )  # either empty or exactly one element
                    try:
                        ...
                        fatal_check(
                            len(list(resource_type)) == 0
                        )  # <collection> must have no children
                        fatal_check(
                            not p.text
                        )  # there must not be text in front of <collection>, FIXME: this may be relaxed later to allow whitespaces
                        fatal_check(
                            not resource_type.text
                        )  # there must not be text inside <collection>
                        fatal_check(
                            not resource_type.tail
                        )  # there must not be text behind <collection>, FIXME: this may be relaxed later to allow whitespaces
