...
def main(step):
    ...
    filename=create_hashfile(d,size=int(5.5*MB))

    # careful with the chunk size...
    r=chunk_file_upload(filename,URL,chunk_size=1*MB,android_client_bug_900=True)
    file_download(os.path.basename(filename),URL,d)
    analyse_hashfiles(d)

    # upload again matching the existing etag
    r=chunk_file_upload(filename,URL,chunk_size=1*MB,android_client_bug_900=True,header_if_match=r.headers['ETag'])
    analyse_hashfiles(d)
