...
__doc__ = """ Add nplustwo_nfiles to a directory (two clients) and check consistency after synch.
"""

from smashbox.utilities import *
from smashbox.utilities.hash_files import *

nplustwo_nfiles = int(config.get('nplustwo_nfiles',10))
...
def adder(step):
    ...
    step(2,'Add %s files and check if we still have k1+nplustwo_nfiles after resync'%nplustwo_nfiles)

    for i in range(nplustwo_nfiles):
    ...
    error_check(k1-k0==2*nplustwo_nfiles,'Expecting to have %d files more: see k1=%d k0=%d'%(nplustwo_nfiles,k1,k0))
